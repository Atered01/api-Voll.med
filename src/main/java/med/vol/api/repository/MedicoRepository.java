...
public interface MedicoRepository extends JpaRepository<Medico, Long> {
    ...
    @Query(" select m from Medico m where m.ativo = true and m.especialidade = :especialidade and m.id not in(select c.medico.id from Consulta c where c.data = :data and c.motivoDoCancelamento is null) order by rand() limit 1")
